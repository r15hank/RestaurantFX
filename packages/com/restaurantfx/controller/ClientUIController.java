		...
		return new ClientConnector("localhost", 8080, 
				data -> {
					Platform.runLater(() -> {
			    		...
			    		stage.getIcons().add(new Image(getClass().getResourceAsStream("/resources/food_client.png")));
			    		ack.initOwner(ClientRun.getStage());
						ack.showAndWait();
					});
				});
    	...
    	if(order.getCornProperty()!=0 || order.getBrocoliProperty()!=0 || order.getBeansProperty()!=0) {
    		...
    		if(result.isPresent() && result.get() == yes) {
            	...
            	try {connector.send(orderSend);}
            		...
            		stageDialog.getIcons().add(new Image(getClass().getResourceAsStream("/resources/food_client.png")));
